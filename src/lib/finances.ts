...
export const updateAssetFromExpense = (expenseId: string, assetId: string | undefined, amount: number, date: string, isDelete = false) => {
  ...
  const rawData = localStorage.getItem(getPrefixedKey(SYNC_KEYS.FINANCES_ASSETS));
  if (!rawData) return;

  try {
    let assetsList: Asset[] = validateLocalData(rawData, SYNC_KEYS.FINANCES_ASSETS) || [];
...
export const updateRecipientFromExpense = (expenseId: string, paidToType: string, paidToId: string | undefined, amount: number, date: string, isDelete = false) => {
  ...
  if (paidToType === 'savings') {
    const rawData = localStorage.getItem(getPrefixedKey(SYNC_KEYS.FINANCES_SAVINGS_TARGETS));
    if (rawData) {
      try {
        let goals: any[] = validateLocalData(rawData, SYNC_KEYS.FINANCES_SAVINGS_TARGETS) || [];
  ...
  if (paidToType === 'emergency') {
    const rawData = localStorage.getItem(getPrefixedKey(SYNC_KEYS.FINANCES_EMERGENCY_FUND));
    if (rawData) {
      try {
        const fund: any = validateLocalData(rawData, SYNC_KEYS.FINANCES_EMERGENCY_FUND);
        if (!fund) return;
...
export const updateLiabilityFromExpense = (expenseId: string, liabilityId: string | undefined, amount: number, date: string, isDelete = false) => {
  if (typeof window === 'undefined') return;
  const rawData = localStorage.getItem(getPrefixedKey(SYNC_KEYS.FINANCES_LIABILITIES));
  if (!rawData) return;

  try {
    let liabilities: Liability[] = validateLocalData(rawData, SYNC_KEYS.FINANCES_LIABILITIES) || [];
