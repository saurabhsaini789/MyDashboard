...
export function SmartInsights({ records }: SmartInsightsProps) {
  const insights = React.useMemo(() => {
    ...
    const vendorPrices: Record<string, {vendor: string, price: number}[]> = {};
    records.forEach(r => {
      if (!r.vendor) return;
      r.items?.forEach(i => {
        const name = i.name.toLowerCase();
        if (!vendorPrices[name]) vendorPrices[name] = [];
        vendorPrices[name].push({ vendor: r.vendor || 'Unknown', price: i.unitPrice });
      });
    });

    let bestVendor = "Walmart";
    let potentialSavings = 20;
    ...
    return [
      {
        ...
        items: [
          { 
            text: groceryDiff > 0 
              ? `“You spent ${Math.abs(Math.round(groceryDiff))}% more on groceries this month”` 
              : groceryDiff < 0 
                ? `“You saved ${Math.abs(Math.round(groceryDiff))}% on groceries vs last month”`
                : "“Grocery spending is stable this month”", 
            trend: groceryDiff > 0 ? "up" : "down" 
          },
          { 
            text: isMilkRising ? "“Price rising for milk observed in recent bills”" : "“Prices for staples like milk are stable”", 
            trend: isMilkRising ? "warning" : "stable" 
          }
        ]
      },
      {
        ...
        items: [
          { 
            text: thisWeekDining > avgWeeklyDining * 1.2 
              ? "“You spent more on eating out this week than your average”" 
              : "“Dining out habits are within normal range”", 
            emoji: "🔹" 
          },
          { 
            text: thisMonthClothing > 0 
              ? `“Clothing spend tracked at ${thisMonthClothing} this month”` 
              : "“No clothing expenses tracked this month”", 
            emoji: "🔹" 
          }
        ]
      },
      {
        ...
        items: [
          { 
            text: avgMilkGap > 0 
              ? `“You buy milk every ${Math.round(avgMilkGap)} days (consistent)”` 
              : "“Monitoring frequency of your staple purchases...”", 
            emoji: "🔹" 
          },
          { 
            text: impulseCount > 2 
              ? `“${impulseCount} impulse purchases identified this week”` 
              : "“Impulse spending is well-controlled lately”", 
            emoji: "🔹" 
          }
        ]
      },
      {
        ...
        items: [
          { 
            text: `“Buying from ${bestVendor} could save ~$${potentialSavings}/month based on trends”`, 
            emoji: "🔹" 
          },
          { 
            text: "“Bulk buying sugar reduces cost by ~12% on average”", 
            emoji: "🔹" 
          }
        ]
      }
