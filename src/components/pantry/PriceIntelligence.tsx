...
export function PriceIntelligence({ records }: PriceIntelligenceProps) {
  ...
  React.useEffect(() => {
    const saved = localStorage.getItem('pantry-price-intelligence-view');
    if (saved === 'table' || saved === 'grid') setViewMode(saved);
  }, []);

  const toggleViewMode = (mode: 'grid' | 'table') => {
    setViewMode(mode);
    localStorage.setItem('pantry-price-intelligence-view', mode);
  };
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-zinc-50 dark:bg-zinc-950/50 border-b border-zinc-100 dark:border-zinc-800">
              <th className="p-4 px-6 text-[11px] uppercase text-zinc-500 font-bold tracking-wider">Item</th>
              <th className="p-4 px-6 text-[11px] uppercase text-zinc-500 font-bold tracking-wider">Last Purchase</th>
              <th className="p-4 px-6 text-[11px] uppercase text-zinc-500 font-bold tracking-wider text-right">Avg / Lowest</th>
              <th className="p-4 px-6 text-[11px] uppercase text-zinc-500 font-bold tracking-wider text-right">Trend</th>
              <th className="p-4 px-6 text-[11px] uppercase text-zinc-500 font-bold tracking-wider text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {filteredStats.map(stat => (
              <tr key={stat.name} onClick={() => setActiveItemStats(stat)} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors group cursor-pointer border-b border-zinc-50 dark:border-zinc-800/50 last:border-0">
                <td className="p-4 px-6">
                  <div className="flex flex-col gap-0.5">
                    <span className="font-semibold text-zinc-900 dark:text-zinc-100">{stat.name}</span>
                    <span className="text-[10px] uppercase font-bold text-zinc-400">{stat.history.length} logs</span>
                  </div>
                </td>
                <td className="p-4 px-6">
                  <div className="flex flex-col gap-0.5">
                    <div className="flex items-center gap-1.5">
                      <span className="font-bold text-zinc-900 dark:text-zinc-100">
                        ${stat.lastPurchase.price.toLocaleString("en-CA", { minimumFractionDigits: 1, maximumFractionDigits: 3 })}
                      </span>
                      <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-tight">/ {stat.lastPurchase.size || 'unit'}</span>
                    </div>
                    <span className="text-xs text-zinc-500 truncate max-w-[150px]">at {stat.lastPurchase.vendor}</span>
                  </div>
                </td>
                <td className="p-4 px-6 text-right">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">
                      Avg: ${stat.averagePrice.toLocaleString("en-CA", {maximumFractionDigits:2})}
                    </span>
                    <span className="text-xs font-bold text-teal-600 dark:text-teal-400">
                      Low: ${stat.lowestPrice.price.toLocaleString("en-CA", {maximumFractionDigits:2})}
                    </span>
                  </div>
                </td>
                <td className="p-4 px-6 text-right">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold ${ stat.priceTrend > 3 ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400' : stat.priceTrend < -3 ? 'bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400' }`}>
                    {stat.priceTrend > 0 ? '↑' : stat.priceTrend < 0 ? '↓' : ''} {Math.abs(stat.priceTrend).toFixed(1)}% vs avg
                  </span>
                </td>
                <td className="p-4 px-6 text-right">
                  <button 
                    className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-xl transition-all text-zinc-400 hover:text-amber-600 dark:hover:text-amber-400"
                  >
                    <Sparkles size={18} />
                  </button>
                </td>
     ...
     <div className="relative z-10 w-full md:w-auto flex items-center gap-3">
       ...
       <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl border border-zinc-200 dark:border-zinc-700">
         <button 
           ...
           className={`p-1.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-zinc-700 shadow-sm text-amber-600 dark:text-amber-400' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
           title="Grid View"
         >
           <LayoutGrid size={18} />
         </button>
         <button 
           ...
           className={`p-1.5 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white dark:bg-zinc-700 shadow-sm text-amber-600 dark:text-amber-400' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}
           title="Table View"
         >
           <List size={18} />
         </button>
 ...
 <h4 className="font-semibold text-[17px] text-zinc-900 dark:text-zinc-100 truncate" title={stat.name}>{stat.name}</h4>
 <span className="text-[11px] uppercase font-semibold text-zinc-500 dark:text-zinc-400">{stat.history.length} Logs</span>
 </div>
 <div className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap ${ stat.priceTrend > 3 ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400' : stat.priceTrend < -3 ? 'bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400' }`}>
 {stat.priceTrend > 0 ? '↑' : stat.priceTrend < 0 ? '↓' : ''} {Math.abs(stat.priceTrend).toFixed(1)}% vs avg
 </div>
 </div>
 ...
 <span className="text-base font-semibold text-zinc-400 mr-1">$</span>{stat.lastPurchase.price.toLocaleString("en-CA", { minimumFractionDigits: 1, maximumFractionDigits: 3 })}
 <span className="text-sm font-semibold text-zinc-400 ml-1">/ {stat.lastPurchase.size || 'unit'}</span>
 </span>
 <span className="text-xs text-zinc-600 dark:text-zinc-300 uppercase font-semibold mt-1 truncate max-w-[150px]" title={stat.lastPurchase.vendor}>
 ...
 <span className="text-[11px] text-zinc-600 dark:text-zinc-300 font-semibold uppercase">Average</span>
 ...
 <span className="text-[11px] text-zinc-600 dark:text-zinc-300 font-semibold uppercase">Lowest</span>
 ...
 <span key={idx} className={`text-xs font-semibold uppercase px-2 py-1 rounded-[8px] truncate max-w-full ${ insight.type === 'positive' ? 'bg-teal-50 dark:bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-100 dark:border-teal-500/20' : insight.type === 'negative' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-500/20' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700' }`} title={insight.text}>
 {insight.text}
 </span>
 ))}
 {stat.smartInsights.length > 2 && (
 <span className="text-xs font-semibold uppercase px-2 py-1 rounded-[8px] bg-zinc-100 dark:bg-zinc-800 text-zinc-500 border border-zinc-200 dark:border-zinc-700">
 +{stat.smartInsights.length - 2}
 </span>
 )}
 </div>
 ...
 <span className="text-[11px] uppercase font-semibold text-zinc-400">Lowest</span>
 ...
 <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-100 dark:border-zinc-800 flex flex-col gap-2">
 <span className="text-[11px] uppercase font-semibold text-zinc-400">Average</span>
 ...
 <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-100 dark:border-zinc-800 flex flex-col gap-2">
 <span className="text-[11px] uppercase font-semibold text-zinc-400">Trend</span>
 ...
 <h4 className="text-[11px] uppercase font-semibold text-zinc-400 pl-2">Smart Insights</h4>
 <div className="flex flex-col gap-2">
 {activeItemStats.smartInsights.map((insight, idx) => (
 <div key={idx} className={`p-4 rounded-2xl border flex items-center gap-3 text-sm font-semibold ${ insight.type === 'positive' ? 'bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-300 border-teal-100 dark:border-teal-500/20' : insight.type === 'negative' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-300 border-rose-100 dark:border-rose-500/20' : 'bg-zinc-50 dark:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700' }`}>
 ...
 {/* History Table */}
 <div className="space-y-3">
 <h4 className="text-[11px] uppercase font-semibold text-zinc-400 pl-2">Price History</h4>
 ...
 <tr>
 <th className="p-4 px-6 text-xs uppercase text-zinc-600 dark:text-zinc-300 font-semibold">Date</th>
 <th className="p-4 px-6 text-xs uppercase text-zinc-600 dark:text-zinc-300 font-semibold">Store</th>
 <th className="p-4 px-6 text-xs uppercase text-zinc-600 dark:text-zinc-300 font-semibold text-right">Details</th>
 <th className="p-4 px-6 text-xs uppercase text-zinc-600 dark:text-zinc-300 font-semibold text-right">Unit Price</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td className="p-4 px-6 text-sm font-semibold text-zinc-900 dark:text-white">
 {h.vendor}
 </td>
 <td className="p-4 px-6 text-xs text-zinc-500 dark:text-zinc-400 text-right">
 {h.brand ? <span className="font-semibold mr-2">{h.brand}</span> : null}
 {h.size || ''}
 </td>
 <td className="p-4 px-6 text-sm font-semibold text-zinc-900 dark:text-white text-right">
