...
export function PantryEntryModal({ isOpen, date, recordsOnDate, onClose, onUpdateRecords, allRecords, initialRecord, initialTab }: PantryEntryModalProps) {
  ...
  const headerControls = (
    <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-full">
      <button type="button" onClick={() => setActiveTab('list')} className={`px-4 py-2 rounded-full text-[10px] uppercase font-bold transition-all ${activeTab === 'list' ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-400'}`}>Records</button>
      <button type="button" onClick={() => setActiveTab('form')} className={`px-4 py-2 rounded-full text-[10px] uppercase font-bold transition-all ${activeTab === 'form' ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-400'}`}>Create</button>
    </div>
  );
        ...
        {activeTab === 'list' ? (
          <div className="space-y-4">
            {recordsOnDate.length === 0 ? <div className="py-20 text-center font-bold text-zinc-300 uppercase">No records</div> : (
              recordsOnDate.map(record => (
                <div key={record.id} onClick={() => startEdit(record)} className="p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-100 hover:border-teal-500 transition-all cursor-pointer">
          ...
          <div className="space-y-8">
            <FormSection title="Basic Details" accentColor="teal">
              <input type="text" required value={vendor} onChange={e => setVendor(e.target.value)} placeholder="Shop / Vendor Name" className="bg-zinc-50 dark:bg-zinc-900 border-none px-4 py-3 rounded-2xl text-sm font-bold w-full outline-none"/>
              <select value={paidFromId} onChange={e => setPaidFromId(e.target.value)} className="bg-zinc-50 dark:bg-zinc-900 border-none px-4 py-3 rounded-2xl text-sm font-bold w-full outline-none">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center"><h3 className="text-[10px] font-bold uppercase text-teal-600">Itemized Bill</h3><button type="button" onClick={handleAddItem} className="text-[10px] font-bold text-zinc-400 uppercase border border-zinc-100 px-3 py-1 rounded-full">+ ADD ITEM</button></div>
              {items.map(item => (
                <div key={item.id} className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <input placeholder="Item Name" value={item.name} onChange={e => updateItem(item.id, 'name', e.target.value)} className="bg-white px-3 py-2 rounded-xl text-xs font-bold outline-none"/>
                    <input type="number" placeholder="Price" value={item.unitPrice} onChange={e => updateItem(item.id, 'unitPrice', parseFloat(e.target.value))} className="bg-white px-3 py-2 rounded-xl text-xs font-bold outline-none"/>
                  </div>
                </div>
              ))}
            </div>
