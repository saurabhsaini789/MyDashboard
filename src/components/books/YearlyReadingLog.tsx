...
export function YearlyReadingLog({ onPromote }: { onPromote?: (book: LogBookEntry, language: 'English' | 'Hindi') => void }) {
 ...
 <Text variant="title" as="div" className="text-2xl min-w-[100px] text-center">
 {currentYear}
 </Text>
 <button 
 onClick={() => setCurrentYear(y => y + 1)}
 ...
 className="w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl pl-10 pr-4 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-teal-500/50 transition-all placeholder:text-zinc-400"
 />
 </div>
 </div>
 ...
 {/* Grid Header */}
 <div className="grid grid-cols-12 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/10">
 <Text variant="label" as="div" className="col-span-2 px-6 py-4 border-r border-zinc-100 dark:border-zinc-800 text-center font-semibold">Month</Text>
 <Text variant="label" as="div" className="col-span-5 px-6 py-4 border-r border-zinc-100 dark:border-zinc-800 flex items-center justify-center gap-2 font-semibold">
 English
 </Text>
 <Text variant="label" as="div" className="col-span-5 px-6 py-4 flex items-center justify-center gap-2 font-semibold">
 Hindi
 </Text>
 </div>
 ...
 <Text variant="label" as="div" className="col-span-2 px-6 py-4 border-r border-zinc-100 dark:border-zinc-800 flex items-center justify-center font-semibold dark:text-zinc-600 uppercase">
 {month.slice(0, 3)}
 </Text>
 ...
 {/* Card View (Mobile) */}
 <div className={`block lg:hidden transition-all duration-500 ease-in-out overflow-hidden ${ (isMobileExpanded || searchQuery) ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0' }`}>
 ...
 <Text variant="label" as="span" className="font-semibold">{month}</Text>
 </div>
 ...
 <div className="flex items-center justify-between">
 <Text variant="label" as="span" className="font-semibold flex items-center gap-2">English</Text>
 {!searchQuery && (
 <button 
 onClick={() => openAddModal(month, 'english')}
 ...
 <Text variant="bodySmall" muted as="span" className="text-xs italic">No entries</Text>
 ...
 <div className="space-y-3">
 <div className="flex items-center justify-between">
 <Text variant="label" as="span" className="font-semibold flex items-center gap-2">Hindi</Text>
 {!searchQuery && (
 <button 
 onClick={() => openAddModal(month, 'hindi')}
 className="w-6 h-6 flex items-center justify-center rounded-lg border border-dashed border-zinc-200 dark:border-zinc-800 text-zinc-300"
...
function EditableBookRow({ book, onUpdate, onRemove, placeholder }: { 
 ...
 <button 
 className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all border ${ book.status === 'None' ? 'border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-300' : 'border-transparent bg-zinc-100 dark:bg-zinc-800 shadow-sm' }`}
 >
 ...
 className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold uppercase transition-colors ${ book.status === opt ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white' : 'text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 hover:text-zinc-900 dark:hover:text-white' }`}
 >
 ...
 className="w-full bg-zinc-100 dark:bg-zinc-800 border-none rounded-md px-2 py-1 text-xs font-semibold focus:ring-2 focus:ring-teal-500/30 text-zinc-900 dark:text-white"
 />
 <input
 type="text"
 value={localAuthor}
 ...
 onKeyDown={(e) => e.key === 'Enter' && handleSubmit()}
 placeholder="Author..."
 className="w-full bg-zinc-50 dark:bg-zinc-900/50 border-none rounded-md px-2 py-0.5 text-[11px] font-semibold uppercase focus:ring-1 focus:ring-zinc-500/30 text-zinc-500 dark:text-zinc-400"
 />
 </div>
 ) : (
 <div 
 onClick={() => setIsEditing(true)}
 className="cursor-pointer transition-colors flex items-baseline gap-1.5 min-w-0"
 >
 <Text variant="title" as="span" className={`text-base leading-tight truncate ${ book.title ? 'text-zinc-800 dark:text-zinc-200' : 'text-zinc-300 dark:text-zinc-700 italic font-normal' }`}>
 {book.title || placeholder}
 </Text>
 {book.author && (
 <Text variant="label" as="span" className="font-semibold text-zinc-400 dark:text-zinc-600 truncate">
 — {book.author}
 </Text>
 )}
 </div>
