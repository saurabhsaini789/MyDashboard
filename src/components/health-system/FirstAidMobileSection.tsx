  ...
  return (
    <section className="w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 px-2">
        <SectionTitle>First Aid - Mobile</SectionTitle>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative group flex-1 min-w-[200px]">
            ...
            <input 
              type="text" 
              placeholder="Search mobile kit..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="bg-zinc-100 dark:bg-zinc-800 text-xs font-bold rounded-2xl h-[54px] pl-12 pr-4 border-none w-full outline-none focus:ring-2 focus:ring-emerald-600/20 transition-all font-bold"
            />
          </div>
          <button onClick={openAddModal} className="bg-zinc-900 dark:bg-zinc-800 text-white dark:text-zinc-100 text-xs font-bold px-6 h-[54px] rounded-2xl hover:scale-105 transition-all w-full md:w-auto border border-transparent dark:border-zinc-700">ADD MOBILE ITEM</button>
        </div>
      </div>
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-2xl border-2 border-l-[6px] bg-white dark:bg-zinc-900/40 mb-8 shadow-sm gap-4 ${expiredCount > 0 ? 'border-rose-200 dark:border-rose-900 border-l-rose-500' : missingCount > 0 ? 'border-amber-200 dark:border-amber-900 border-l-amber-500' : 'border-emerald-200 dark:border-emerald-900 border-l-emerald-500'}`}>
        <div className="flex gap-4 items-center">
          ...
          <Text variant="body" className="font-bold">{expiredCount > 0 ? 'Attention - Expired mobile kit' : missingCount > 0 ? 'Restock mobile kit' : 'Mobile Kit Ready'}</Text>
        </div>
      ...
      <Modal isOpen={isModalOpen} onClose={()=>setIsModalOpen(false)} title="Mobile Record" onSubmit={handleSubmit}>
        <DynamicForm
        ...
        />
        {editingItem && <button type="button" onClick={()=>deleteItem(editingItem.id)} className="mt-4 text-xs font-bold text-rose-500">DELETE RECORD</button>}
      </Modal>
    </section>
  );
}
