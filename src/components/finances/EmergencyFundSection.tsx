...
export function EmergencyFundSection() {
  const data = useStorageSubscription<EmergencyFundData | null>(SYNC_KEYS.FINANCES_EMERGENCY_FUND, null);
  ...
  useEffect(() => {
    ...
    if (!data && isLoaded) {
      const initialData: EmergencyFundData = {
        targetAmount: 30000,
      ...
      setSyncedItem(SYNC_KEYS.FINANCES_EMERGENCY_FUND, JSON.stringify(initialData));
    }
  }, [data, isLoaded]);

  if (!isLoaded || !data) return null;

  const updateData = (newData: EmergencyFundData) => {
    setSyncedItem(SYNC_KEYS.FINANCES_EMERGENCY_FUND, JSON.stringify(newData));
  };
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 w-full transition-all duration-700 animate-in fade-in slide-in-from-bottom-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 px-1 md:px-2">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4 justify-start sm:justify-end">
          <button 
            ...
            className="bg-zinc-50 text-zinc-600 border border-zinc-100/50 dark:bg-zinc-800/30 dark:text-zinc-400 dark:border-zinc-800 font-semibold uppercase text-xs px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-2xl transition-all"
          >
            Settings
          </button>
          <button 
            onClick={() => setIsContributionModalOpen(true)}
            className="bg-amber-500 text-white text-xs px-5 sm:px-8 py-2.5 sm:py-3.5 rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-sm dark:shadow-none font-semibold"
          >
            Log
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900/60 border border-l-4 border-amber-100/50 dark:border-amber-900/30 rounded-2xl p-4 sm:p-6 lg:p-8 shadow-sm transition-all duration-500 relative overflow-hidden group">
        ...
        <div className={`sm:hidden flex items-center justify-between relative z-10 mb-4 ${isCollapsed ? 'flex' : 'hidden'}`}>
          <div className="flex flex-col gap-0.5">
            <span className="text-xs text-zinc-500 dark:text-zinc-300 uppercase font-medium">Coverage</span>
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10 sm:grid ${isCollapsed ? 'hidden' : 'grid'}`}>
          {/* Progress */}
          <div className="flex flex-col gap-4 justify-between bg-white dark:bg-zinc-900/40 rounded-2xl p-5 border border-l-4 border-zinc-100 dark:border-zinc-800 shadow-sm">
            <div className="flex flex-col gap-1">
              <span className="text-xl md:text-2xl text-zinc-900 dark:text-white font-bold">
                {monthsCovered < 10 ? monthsCovered.toFixed(1) : Math.floor(monthsCovered)} / {targetMonths < 10 && targetMonths > 0 ? targetMonths.toFixed(1) : Math.floor(targetMonths)} Months Saved
              </span>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between items-end">
                <span className="text-xs text-zinc-600 dark:text-zinc-300 font-semibold uppercase">Progress</span>
                <span className={`text-xs uppercase px-3 py-1 rounded-full ${monthsCovered >= 6 ? 'text-orange-600 bg-orange-100 dark:text-orange-400 dark:bg-orange-500/20' : monthsCovered >= 3 ? 'text-amber-600 bg-amber-100 dark:text-amber-400 dark:bg-amber-500/20' : 'text-rose-600 bg-rose-100 dark:text-rose-400 dark:bg-rose-500/20'} font-semibold`}>
          ...
          {/* Tracker */}
          <div className="flex flex-col gap-4 bg-white dark:bg-zinc-900/40 shadow-sm rounded-2xl p-5 border border-l-4 border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center justify-between pb-2 border-b border-zinc-200/50 dark:border-zinc-700/50">
              <h4 className="text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase">Contribution Tracker</h4>
              <button onClick={() => setIsHistoryModalOpen(true)} className="text-xs text-amber-600 dark:text-amber-400 uppercase font-semibold hover:underline">View All</button>
            </div>
            <div className="flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-1 flex-1 min-h-[140px]">
              {(recentContributions || []).length > 0 ? recentContributions.map(c => (
                ...
                <div className="flex flex-col items-center justify-center p-4 bg-white/50 dark:bg-zinc-800/30 rounded-2xl border border-dashed border-zinc-200/80 dark:border-zinc-700 h-full">
                  <span className="text-xs text-zinc-500 dark:text-zinc-300 uppercase text-center font-medium">No contributions yet</span>
                </div>
              )}
            </div>
          </div>
      ...
      <Modal isOpen={isContributionModalOpen} onClose={() => setIsContributionModalOpen(false)} title="Fuel factor" onSubmit={handleAddContribution}>
        <DynamicForm
          sections={[{ id: 'fuel', title: 'Log', fields: [{ name: 'amount', label: 'Amount', type: 'number', required: true }]}]}
          formData={{ amount: contributionAmount }}
          onChange={(_, v) => setContributionAmount(v)}
        />
      </Modal>

      <Modal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} title="Fuel records">
