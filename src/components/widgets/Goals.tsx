...
export function Goals() {
  ...
  const [editProjectImportant, setEditProjectImportant] = useState(false);
  ...
  const initiateProjectEdit = () => {
    if (!selectedProject) return;
    setEditProjectTitle(selectedProject.title);
    setEditProjectDate(selectedProject.dueDate);
    setEditProjectImportant(selectedProject.isImportant);
    setIsEditingProject(true);
  };

  const saveProjectEdits = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedProject || !editProjectTitle.trim()) return;

    setProjects(projects.map(p =>
      p.id === selectedProject.id
        ? { ...p, title: editProjectTitle.trim(), dueDate: editProjectDate, isImportant: editProjectImportant }
        : p
    ));
  ...
  return (
    <div className="w-full relative">
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-zinc-900/40 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedProject(null)}>
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Modal Header */}
            {isEditingProject ? (
              <form onSubmit={saveProjectEdits} className="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                <input
                  autoFocus
                  type="text"
                ...
                <div className="flex flex-wrap gap-4 items-end">
                  ...
                  <div className="flex flex-col gap-1.5 flex-1 min-w-[150px]">
                    ...
                    <button
                      type="button"
                      onClick={() => setEditProjectImportant(!editProjectImportant)}
                      className={`w-full flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition-colors border ${editProjectImportant
                          ? 'bg-orange-50 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-500/30'
                          : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 border-transparent hover:bg-zinc-200 dark:hover:bg-zinc-700'
                        }`}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill={editProjectImportant ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                      Important
                    </button>
                  </div>
                </div>
                <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                  <button type="button" onClick={cancelProjectEdit} className="px-4 py-2 text-sm font-medium text-zinc-500 hover:text-zinc-900 dark:hover:text-white">Cancel</button>
                ...
                <div className="pr-4">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-500/10 px-2 py-1 rounded-md">
                      {selectedProject.bucketId}
                    </span>
                    {(() => {
                      const info = getPriorityInfo(selectedProject);
                      return (
                        <span className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-md ${info.classes.split(' shadow')[0]}`}>
                          {info.label}
                        </span>
                      );
                    })()}
                  ...
                  <div className="flex items-center gap-3">
                    <h2 className={`text-2xl font-extrabold leading-tight ${selectedProject.isCompleted ? 'text-zinc-400 dark:text-zinc-600 line-through' : 'text-zinc-900 dark:text-white'}`}>
                  ...
                  <div className="flex items-center gap-2 mt-2 text-sm text-zinc-500 dark:text-zinc-400 font-medium">
                    ...
                    Due {new Date(selectedProject.dueDate + 'T12:00:00').toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    onClick={initiateProjectEdit}
            ...
            <div className="p-6 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/30">
              <form onSubmit={handleCreateTask} className="relative flex items-center">
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-zinc-900 dark:bg-white text-white dark:text-black rounded-lg hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              <div className="mt-4 flex items-center justify-between">
                <button
                  onClick={() => handleToggleProjectCompletion(selectedProject.id)}
                  className={`text-xs font-bold py-2 px-4 rounded-md transition-colors ${selectedProject.isCompleted
                ...
                >
                  {selectedProject.isCompleted ? 'Mark as Incomplete' : 'Mark as Complete'}
                </button>
                <button
